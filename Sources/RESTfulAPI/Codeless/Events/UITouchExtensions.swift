...
internal extension UITouch {
    ...
    func attemptReinforcement() {
        if let view = self.view,
            ...
            let reinforcements = DopamineVersion.current.codelessReinforcementFor(sender: senderClassname, target: targetName, selector: selectorName)  { reinforcement in
            ...
            DopamineChanges.shared.delegate?.attemptedReinforcement?(senderInstance: self, targetInstance: view, actionSelector: selectorName, reinforcements: reinforcements)
        }
    }
