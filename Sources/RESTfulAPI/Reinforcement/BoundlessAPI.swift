...
//        }
        ...
        let task = session.dataTask(with: request, completionHandler: { responseData, responseURL, error in
            ...
            if BoundlessAPI.logCalls { DopeLog.debug("got response:\(responseDict.debugDescription)") }
        })
        
        // send request
        DopeLog.debug("Sending \(type.path) api call")
        if BoundlessAPI.logCalls { DopeLog.debug("with payload: \(payload as AnyObject)") }
        task.resume()
