...
internal class BoundlessProperties : UserDefaultsSingleton {
    ...
    static var current: BoundlessProperties = {
        return BoundlessProperties.convert(from: DopamineKit.testCredentials) ??
...
//        DopeLog.debug("Saved BoundlessProperties to user defaults.")
    }
...
//            DopeLog.debug("Found BoundlessProperties saved in user defaults.")
            self.init(
                appID: appID,
...
//            DopeLog.debug("Invalid BoundlessProperties saved to user defaults.")
            return nil
        }
    }
