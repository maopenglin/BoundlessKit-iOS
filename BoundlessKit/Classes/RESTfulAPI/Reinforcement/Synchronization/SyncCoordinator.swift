...
public class SyncCoordinator {
    ...
    ///
    internal func store(track action: BoundlessAction) {
    ...
    ///
    internal func store(report action: BoundlessAction) {
    ...
    /// that allows time for the backend to generate cartridges
    ///
    ...
    public func performSync() {
        syncOperationQueue.addOperation {
            ...
            if trackShouldSync {
                ...
                BoundlessLog.debug("Syncing because \(syncCause)")
                ...
                Track.current.sync() { status in
                    guard status == 200 || status == 0 else {
                        BoundlessLog.debug("Track failed during sync. Halting sync.")
                        goodProgress = false
                ...
                if reportShouldSync {
                    Report.current.sync() { status in
                        if status == 0 {
                            BoundlessLog.debug("Report has nothing to sync")
                        } else if status == 200 {
                            BoundlessLog.debug("Report successfully synced")
                        } else if status == 400 {
                            BoundlessLog.debug("Flushed outdated actions.")
                        } else {
                            BoundlessLog.debug("Report failed during sync. Halting sync.")
                            goodProgress = false
                            Telemetry.stopRecordingSync(successfulSync: false)
                        }
                    }
                ...
                for (actionID, cartridge) in Cartridge.cartridgeSyncers where goodProgress && cartridge.isTriggered() {
                    cartridge.sync() { status in
                        guard status == 200 || status == 0 || status == 400 else {
                            BoundlessLog.debug("Refresh for \(actionID) failed during sync. Halting sync.")
                            goodProgress = false
                            Telemetry.stopRecordingSync(successfulSync: false)
                            return
                        }
