...
public class BoundlessAPI : NSObject{
    ...
    ///
    internal static func track(_ actions: [BoundlessAction], completion: @escaping ([String:Any]) -> ()){
    ...
    ///     - completion: A closure to handle the JSON formatted response.
    ///
    internal static func report(_ actions: [BoundlessAction], completion: @escaping ([String:Any]) -> ()){
        var payload = BoundlessProperties.current.apiCredentials
    ...
    internal static func refresh(_ actionID: String, completion: @escaping ([String:Any]) -> ()){
        ...
        BoundlessLog.debug("Refreshing \(actionID)...")
        shared.send(call: .refresh, with: payload, completion: completion)
    }
    
    /// Send sync overviews and raised exceptions to the `/sync` path to increase service quality
    ///
    /// - parameters:
    ///     - syncOverviews: The array of SyncOverviews to send
    ///     - exceptions: The array of BoundlessExceptions to send
    ///     - completion: A closure to handle the JSON formatted response.
    ///
    internal static func sync( syncOverviews: [SyncOverview], exceptions: [BoundlessException], completion: @escaping ([String:Any]) -> ()){
        var payload = BoundlessProperties.current.apiCredentials
...
//        }
        ...
        let task = session.dataTask(with: request, completionHandler: { responseData, responseURL, error in
            ...
            if BoundlessAPI.logCalls { BoundlessLog.debug("got response:\(responseDict.debugDescription)") }
        })
        
        // send request
        BoundlessLog.debug("Sending \(type.path) api call")
        if BoundlessAPI.logCalls { BoundlessLog.debug("with payload: \(payload as AnyObject)") }
        task.resume()
