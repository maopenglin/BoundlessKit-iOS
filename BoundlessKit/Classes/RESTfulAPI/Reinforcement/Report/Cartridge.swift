...
internal class Cartridge : NSObject, NSCoding {
    ...
    private static let cartridgeActionIDSetKey = "BoundlessAIReinforceableActionIDSet"
    ...
    private var defaultsKey: String { get { return "BoundlessAICartridge_For_\(self.actionID)" } }
    ...
    ///
    /// - parameters:
    ///     - actionID: The name of an action configured on the Boundless Dashboard.
    ...
    init(actionID: String, initialSize: Int=0, timerStartsAt: Int64 = 0, timerExpiresIn: Int64 = 0) {
        self.customerVersion = BoundlessVersion.current.versionID
...
//                BoundlessLog.debug("Erased outdated cartridge.")
            }
        }
...
//            BoundlessLog.debugLog("Decoded cartridge with actionID:\(actionID) reinforcementDecisions:\(reinforcementDecisions.count) initialSize:\(initialSize) timerStartsAt:\(timerStartsAt) timerExpiresIn:\(timerExpiresIn)")
        } else {
            return nil
        }
    }
    ...
    func encode(with aCoder: NSCoder) {
        ...
        //        BoundlessLog.debugLog("Encoded cartridge with actionID:\(actionID) reinforcementDecisions:\(reinforcementDecisions.count) initialSize:\(initialSize) timerStartsAt:\(timerStartsAt) timerExpiresIn:\(timerExpiresIn)")
    }
    ...
    private func timerDidExpire() -> Bool {
        ...
        //        BoundlessLog.debugLog("Cartridge \(actionID) expires in \(timerStartsAt + timerExpiresIn - currentTime)ms so \(isExpired ? "does" : "doesn't") need to sync...")
        return isExpired
    }
    ...
    private func isCapacityToSync() -> Bool {
        ...
        //        BoundlessLog.debugLog("Cartridge for \(actionID) has \(count)/\(initialSize) decisions so \(result ? "does" : "doesn't") need to sync since a cartridge requires at least \(Cartridge.minimumSize) decisions or \(Cartridge.capacityToSync*100)%% capacity.")
        return result
    }
    ...
    /// Retrieves a new cartridge of reinforcement decisions
    ///
    /// - parameters:
    ///     - completion(Int): Returns the status code, or 0 if the cartridge is already being synced by another thread.
    ///
    func sync(completion: @escaping (Int) -> () = { _ in }) {
        DispatchQueue.global(qos: DispatchQoS.QoSClass.userInitiated).async{
            ...
            BoundlessAPI.refresh(self.actionID) { response in
...
//                        BoundlessLog.debug("✅ \(self.actionID) refreshed!")
                    } else if responseStatusCode == 400 {
//                        BoundlessLog.debug("Cartridge contained outdated actionID. Flushing.")
                        Cartridge.flush(self)
                    }
                    completion(responseStatusCode)
                } else {
                    BoundlessLog.debug("❌ Could not read cartridge for (\(self.actionID))")
                    completion(404)
                }
            }
        }
