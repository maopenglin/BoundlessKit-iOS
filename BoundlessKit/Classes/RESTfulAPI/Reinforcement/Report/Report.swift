...
internal class Report : UserDefaultsSingleton {
    ...
    fileprivate static var _current: Report? =  { return UserDefaults.boundless.unarchive() }()
    {
        didSet {
    ...
    @objc private var reportedActions: [BoundlessAction]
    ...
    ///
    private init(versionID: String? = BoundlessVersion.current.versionID, reportedActions: [BoundlessAction] = [], timerStartsAt: Int64 = Int64( 1000*NSDate().timeIntervalSince1970 ), timerExpiresIn: Int64 = 172800000) {
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        if let versionID = aDecoder.decodeObject(forKey: #keyPath(Report.versionID)) as? String?,
            let reportedActions = aDecoder.decodeObject(forKey: #keyPath(Report.reportedActions)) as? [BoundlessAction] {
...
//            BoundlessLog.debug("Decoded report with versionID:\(versionID ?? "nil") reportedActions:\(reportedActions.count) sizeToSync:\(BoundlessConfiguration.current.reportBatchSize) timerStartsAt:\(timerStartedAt) timerExpiresIn:\(timerExpiresIn)")
        } else {
            return nil
        }
    }
    ...
    override func encode(with aCoder: NSCoder) {
        ...
        //        BoundlessLog.debugLog("Encoded report with reportedActions:\(reportedActions.count) sizeToSync:\(sizeToSync) timerStartsAt:\(timerStartsAt) timerExpiresIn:\(timerExpiresIn)")
    }
    ...
    private func timerDidExpire() -> Bool {
        ...
        //        BoundlessLog.debugLog("Report timer expires in \(timerStartsAt + timerExpiresIn - currentTime)ms so the timer \(isExpired ? "will" : "won't") trigger a sync...")
        return isExpired
    }
    ...
    private func batchIsFull() -> Bool {
        ...
        let isBatchSizeReached = count >= BoundlessConfiguration.current.reportBatchSize
        //        BoundlessLog.debugLog("Report has \(count)/\(sizeToSync) actions so the size \(isSize ? "will" : "won't") trigger a sync...")
        return isBatchSizeReached
    }
    
    /// Stores a reported action to be synced at a later time
    ///
    /// - parameters:
...
//                BoundlessLog.debug("No reported actions to sync.")
                completion(0)
                self.updateTriggers()
            } else {
//                BoundlessLog.debug("Sending \(self.reportedActions.count) reported actions...")
                BoundlessAPI.report(self.reportedActions, completion: { response in
                    defer { self.syncInProgress = false }
