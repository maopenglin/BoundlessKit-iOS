...
internal extension UIApplication {
    ...
    func attemptReinforcement(senderInstance: AnyObject, targetInstance: AnyObject, selectorObj: Selector) {
        ...
        BoundlessVersion.current.codelessReinforcementFor(sender: senderClassname, target: targetClassname, selector: selectorName) { reinforcement in
    ...
    private func reinforcementViews(senderInstance: AnyObject, targetInstance: AnyObject, options: [String: Any]) -> [(UIView, CGPoint)]? {
        ...
        case "sender":
            if let view = senderInstance as? UIView {
                ...
                BoundlessLog.error("Could not find sender view for \(type(of: senderInstance))", visual: true)
                return nil
            }
        ...
        case "superview":
            if let senderInstance = senderInstance as? UIView,
                ...
                BoundlessLog.error("Could not find superview", visual: true)
                return nil
            }
        ...
        case "target":
            if let view = targetInstance as? UIView {
                ...
                BoundlessLog.error("Could not find target view", visual: true)
                return nil
            }
        ...
        default:
            BoundlessLog.error("Unknown ViewOption <\(viewOption)>", visual: true)
            return nil
        }
