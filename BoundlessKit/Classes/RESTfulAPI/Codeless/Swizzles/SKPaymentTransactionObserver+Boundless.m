//
//  SKPaymentTransactionObserver+Boundless.m
//  Pods
//
...
@implementation BoundlessPaymentTransactionObserver

+ (void) swizzleSelectors {
    [SwizzleHelper injectSelector:[BoundlessPaymentTransactionObserver class] :@selector(swizzled_addTransactionObserver:) :[SKPaymentQueue class] :@selector(addTransactionObserver:)];
}
...
- (void)swizzled_addTransactionObserver:(id<SKPaymentTransactionObserver>)observer {
    ...
    Class swizzledClass = [BoundlessPaymentTransactionObserver class];
...
- (void)swizzled_paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray<SKPaymentTransaction *> *)transactions {
    ...
    if (transactions && [[BoundlessConfiguration current] storekitObservations]) {
        for (SKPaymentTransaction* transaction in transactions) {
            ...
            [BoundlessKit track:@"SKPaymentTransactionObserver" metaData:@{@"tag": @"updatedTransactions",
