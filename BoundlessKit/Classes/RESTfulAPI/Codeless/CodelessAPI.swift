...
public class CodelessAPI : NSObject {
    ...
    private static var connectionID: String? {
        didSet {
            if connectionID != oldValue {
                BoundlessLog.debug("🔍 \(connectionID != nil ? "C" : "Disc")onnected to visualizer")
            }
            
            if connectionID == nil {
                BoundlessVersion.current.update(visualizer: nil)
    ...
    public static func boot(completion: @escaping () -> () = {}) {
        ...
        payload["inProduction"] = BoundlessProperties.current.inProduction
        payload["currentVersion"] = BoundlessVersion.current.versionID ?? "nil"
        payload["currentConfig"] = BoundlessConfiguration.current.configID ?? "nil"
    ...
    private static func promptPairing() {
        guard !BoundlessProperties.current.inProduction || BoundlessConfiguration.current.integrationMethod != "codeless" else {
            stashSubmits = false
            return
        }
            ...
            submit { payload in
                ...
                DispatchQueue.main.sync {
                    if let view = senderInstance as? UIView,
                        ...
                        NSLog("Cannot create image, please message team@boundless.ai to add support for visualizer snapshots of class type:<\(type(of: senderInstance))>!")
                        payload["senderImage"] = ""
                    }
                }
            }
                ...
                if let connectionID = self.connectionID {
                    ...
                    shared.send(call: .submit, with: payload){ response in
                        ...
                        if response["status"] as? Int != 200 {
                            ...
                            BoundlessVersion.current.update(visualizer: visualizerMappings)
                        } else {
                            BoundlessLog.debug("No visualizer mappings found")
                        }
                    }
                }
            }
    ...
    private func send(call type: CallType, with payload: [String:Any], timeout:TimeInterval = 3.0, completion: @escaping ([String: Any]) -> Void) {
        guard let url = URL(string: type.path) else {
            BoundlessLog.debug("Invalid url <\(type.path)>")
            return
        }
        ...
        let task = URLSession.shared.dataTask(with: request, completionHandler: { responseData, responseURL, error in
            ...
            if responseURL == nil {
                BoundlessLog.debug("❌ invalid response:\(String(describing: error?.localizedDescription))")
            ...
            if let responseData = responseData,
                ...
                BoundlessLog.debug("✅\(type.path) call got empty response.")
                return
            }
            ...
            do {
                ...
                BoundlessLog.debug("❌ Error reading \(type.path) response data: \(String(describing: (responseData != nil) ? String(data: responseData!, encoding: .utf8) : String(describing: responseData.debugDescription)))")
                return
            }
            
            BoundlessLog.debug("✅\(type.path) call")
            if CodelessAPI.logCalls { BoundlessLog.debug("got response:\(responseDict as AnyObject)") }
        })
        
        // send request
        BoundlessLog.debug("Sending \(type.path) api call")
        if CodelessAPI.logCalls { BoundlessLog.debug("with payload: \(payload as AnyObject)") }
        task.resume()
